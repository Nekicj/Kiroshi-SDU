...
@Config
public class TurretControllerLimelight {
    ...
    public static double limelight_kP = 0.3;
    public static double limelight_kD = 0.0002;
    ...
    private TurretMode currentMode = TurretMode.FIELD_ANGLE;
    ...
    private boolean useLimelightTracking = false;
    ...
    public void update(Pose robotPose) {
        ...
        if (limelightValid) {
            if (currentMode != TurretMode.MANUAL && currentMode != TurretMode.LIMELIGHT_TRACKING) {
                useLimelightTracking = true;
            }
        } else {
            useLimelightTracking = false;
        }
        ...
        if (calibrationMode) {
            ...
            if (useLimelightTracking && limelightValid) {
    ...
    private void updateLimelightTrackingMode() {
        if (limelightValid) {
            double correction = -limelightTx * limelight_kP;

            if (Math.abs(limelightTx) > 0.1) {

            }

            setTargetAngle(getCurrentAngle() + correction);
        }
    ...
    public void forceLimelightTracking(boolean force) {
        useLimelightTracking = force;
    }

    public void setLimelightKP(double kp) {
        this.limelight_kP = kp;
    }
    ...
    public void showTelemetry(Telemetry telemetry) {
        ...
        telemetry.addData("Using Limelight", useLimelightTracking ? "YES" : "NO");

        telemetry.addLine("=== LIMELIGHT ===");
        telemetry.addData("Limelight Valid", limelightValid ? "YES" : "NO");
        if (limelightValid) {
            telemetry.addData("Tx", "%.2f°", limelightTx);
            telemetry.addData("Limelight kP", "%.3f", limelight_kP);
        }
    }
}